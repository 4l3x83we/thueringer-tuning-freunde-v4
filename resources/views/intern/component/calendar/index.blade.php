        ...
        <div class="event-box-time-social {{ $calender->kalendertype->typeColor }}" data-aos="flip-down" data-aos-delay="300">
            ...
            <div class="event-box-social">
                @hasanyrole('super_admin|admin')
                    ...
                    @if($calender->team->id !== auth()->user()->id)
                        <a href="{{ route('intern.kalender.edit', $calender->id) }}" class="btn btn-link btn-sm p-0"><em class="bi bi-pen text-black fw-bold pe-0"></em></a>
                    @endif
                @endhasanyrole
                @if($calender->team->id === auth()->user()->id)
                    <a href="{{ route('intern.kalender.edit', $calender->id) }}" class="btn btn-link btn-sm p-0"><em class="bi bi-pen text-black fw-bold pe-0"></em></a>
